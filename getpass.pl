/*  $Id: getpass.pl,v 1.1.1.1 1999/11/18 16:24:43 jan Exp $
...
clear(I) :->
	"Erase typed password"::
	get(I, shadow, Shadow),
	send(I, send_super, clear),
	send(Shadow, clear).
...
make_star_string(Size, S) :-
	new(S, string),
	make_star_string(0, Size, S).

make_star_string(N, N, _) :- !.
make_star_string(N, S, Str) :-
	send(Str, append, '*'),
	NN is N + 1,
	make_star_string(NN, S, Str).
